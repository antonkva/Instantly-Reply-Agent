...
const CACHE_TTL_MS = 60 * 1000; // re-fetch from Airtable at most once a minute

// Table names confirmed to match your Airtable tabs exactly.
...
async function fetchAllRecords(table) {
  ...
  do {
    ...
    offset = data.offset; // present only if there's another page (>100 records)
  } while (offset);
...
async function loadClients() {
  ...
  // Build clients keyed by their Airtable record id first.
  // Field names below match your "Clients" table columns exactly.
  ...
  for (const record of clientRecords) {
    ...
    clientsByRecordId[record.id] = {
      ...
      meeting_link_notes: f["Meeting link?"] ?? "", // e.g. "yes" or "no. Just ask to call them instead"
      calendar_link: f["Calendar Link"] ?? "",
    };
  }

  // Build campaign_id -> full config (client fields + this campaign's ICP/notes).
  const clientsByCampaignId = {};
  for (const record of campaignRecords) {
    const f = record.fields;
    const campaignId = f["Campaign ID"];
    if (!campaignId) continue; // skip incomplete rows rather than crash
