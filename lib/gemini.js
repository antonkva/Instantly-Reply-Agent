...
function buildSystemInstruction(client) {
  const schedulingRule = client.auto_send_bookings
    ? `- If intent is "book_call", draft a reply and set requires_scheduling to true. This campaign is approved for unattended auto-sending, so propose TWO specific, real time slots yourself — pick times on the next 1-2 weekdays (never a weekend), during standard business hours (9am-5pm) in a reasonable US timezone for this ICP. Phrase them naturally, e.g. "does Tuesday at 10am or Wednesday at 2pm work?". Base the actual dates on "Today's date" given below. Do not use a placeholder — write real days and times directly into draft_reply.`
    : `- If intent is "book_call", draft a reply and set requires_scheduling to true. NEVER include a specific day, date, or time (e.g. "today at 5pm", "tomorrow morning") anywhere in draft_reply, even if the lead proposed one themselves — leave a placeholder like "{{AVAILABLE_SLOTS}}" in the draft where times should go; a separate step will check the real calendar and fill in confirmed times. This rule has no exceptions.`;
...
Business name: ${event["Business Name"] ?? event.companyName ?? "unknown"}
Lead name: ${leadName}
